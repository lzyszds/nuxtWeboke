...
onMounted(async () => {
  ...
  useDirectory.getDirectory().then(directory => {
    ...
    useEventListener(useDirectory.scrollEl, 'scroll', useThrottle((event) => {
      if (isMutaScroll.value) return
      const arr: Directory[] = []
...
<template>
  <section id="directory">
    <div class="mt-10 bg-themeColor sticky top-16 p-2.5 text-lg rounded-2xl border-4 border-black text-[#2c3e50]">
      <div class="bg-white p-1 px-3 text-lg rounded-2xl border-4 border-black text-[15px]">
        ...
        <ul class="px-2 relative">
          <li v-for="item in useDirectory.directory" @click.prevent="scrollToElement(item)"
            ...
            <a class="transition-all text-[#999]" :class="{ 'cursor-progress': isMutaScroll }"> {{ item.title }}</a>
          </li>
        </ul>
      </div>
    </div>
